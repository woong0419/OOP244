...
namespace sdds {
	...
	Ship::Ship(const char* sType, Engine arreng[], int Nengine) {
		...
		{
			ship_type = new char[strlen(sType) + 1];

			this->number_engines = Nengine;
			strcpy(this->ship_type, sType);

			eng = new Engine[Nengine];
	...
	bool Ship::empty() const {
		return number_engines == 0 || ship_type==nullptr;
	}
	...
	Ship& Ship::operator +=(Engine e) {
		if (!empty())
		{
			...
			Engine *tempEngine = new Engine[number_engines+1];
			for (int i = 0; i < number_engines; i++)
			{
				tempEngine[i] = eng[i];
			}
			tempEngine[number_engines] = e;
			number_engines += 1;
			delete[] eng;
			eng = nullptr;
			eng = new Engine[number_engines];
			for (int i = 0; i < number_engines; i++)
			{
				eng[i] = tempEngine[i];
			}
